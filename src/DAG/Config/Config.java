...
public class Config
{
			...
			while ((read = reader.read(buffer, 0, BUFFER_LENGTH)) > 0)
			{
				...
				for (int i=0 ; i<read ; i++)
				{
					...
					else
					{
						...
						// If starting a comment, or already in a comment just continue
						if (c == '#' || inComment) {
							inComment = true;
							continue;
						}
						...
						if (c == ';') {
							...
							// Reset the configBuilder ready for the next key-value datum
	...
	public static String[] splitArray(String str) throws ConfigException
	{
		...
		// The first stage checks for matching pairs of brackets and find the bracket depth that

...
public class MCQuery {
	...
	byte[] serverAddress = new byte[] { 127, 0, 0, 1 };
	...
	public MCQuery(byte[] address) {
		this(address, 25565);
	}

	public MCQuery(byte[] address, int port) {
	...
	private byte[] sendUDP(byte[] input) {
		try {
			...
			InetAddress address = InetAddress.getByAddress(serverAddress); //create InetAddress object from the address
